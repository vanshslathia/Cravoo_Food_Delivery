...
const token=userData?.token || tokenFromStorage
const authConfig = {
...
const totalEarning=todayDeliveries.reduce((sum,d)=>sum + d.count*ratePerDelivery,0)
  ...
  const getAssignments=async () => {
    try {
      const result=await axios.get(`${serverUrl}/api/order/get-assignments`,authConfig)
      setAvailableAssignments(Array.isArray(result.data) ? result.data : [])
      console.log("getAssignments response:", result.data)
    } catch (error) {
  ...
  const getCurrentOrder=async () => {
     try {
      const result=await axios.get(`${serverUrl}/api/order/get-current-order`,authConfig)
  ...
  const acceptOrder=async (assignmentId) => {
    try {
      const result=await axios.get(`${serverUrl}/api/order/accept-order/${assignmentId}`,authConfig)
  ...
  useEffect(()=>{
    const socket=getSocket() || initializeSocket(serverUrl)
    if(!socket || userData?.role!=="deliveryBoy") return
    const handleNewAssignment=(data)=>{
      console.log("newAssignment received:", data)
      setAvailableAssignments(prev=>{
        const previous = Array.isArray(prev) ? prev : []
        if (previous.some(a => a.assignmentId === data.assignmentId)) return previous
  ...
  const sendOtp=async () => {
   ...
   const handleTodayDeliveries=async () => {
    ...
    try {
      const result=await axios.get(`${serverUrl}/api/order/get-today-deliveries`,authConfig)
    console.log(result.data)
