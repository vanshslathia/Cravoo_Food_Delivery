...
const boysToShow=availableBoys?.length>0 ? availableBoys : (data?.shopOrders?.availableBoys || [])
const dispatch=useDispatch()
useEffect(()=>{
    setAvailableBoys(data?.shopOrders?.availableBoys || [])
},[data?.shopOrders?.availableBoys])
    const handleUpdateStatus=async (orderId,shopId,status) => {
        try {
            const result=await axios.post(`${serverUrl}/api/order/update-status/${orderId}/${shopId}`,{status},{withCredentials:true})
             const responseStatus=result?.data?.shopOrder?.status || status
             const responseAvailableBoys=result?.data?.availableBoys || []
             dispatch(updateOrderStatus({orderId,shopId,status:responseStatus,availableBoys:responseAvailableBoys}))
             setAvailableBoys(responseAvailableBoys)
...
<div className="mt-3 p-2 border rounded-lg text-sm bg-orange-50 gap-4">
   ...
   {boysToShow?.length>0?(
     boysToShow.map((b,index)=>(
        <div className='text-gray-800' key={b.id || index}>{b.fullName}-{b.mobile}</div>
     ))
